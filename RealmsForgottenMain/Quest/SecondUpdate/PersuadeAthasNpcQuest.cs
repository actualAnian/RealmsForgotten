...
namespace RealmsForgotten.Quest.SecondUpdate
{
    ...
    public class PersuadeAthasNpcQuest : QuestBase
    {
        ...
        private void WaitUntilDecipher()
        {
            ...
            TransferPrisonerAction.Apply(athasScholarHero.CharacterObject, PartyBase.MainParty, Hero.OneToOneConversationHero.PartyBelongedTo.Party);
        ...
        private DialogFlow HideoutBossDialogFix => DialogFlow.CreateDialogFlow("start", 125).NpcLine(
                "{=nYCXzAYH}You! You've cut quite a swathe through my men there, damn you. How about we settle this, one-on-one?")
            .Condition(() =>
                CharacterObject.OneToOneConversationCharacter?.StringId == hideoutBossCharacterId &&
                Settlement.CurrentSettlement == questHideout && goToHideoutLog?.CurrentProgress == 1)
            .GotoDialogState("bandit_hideout_defender");
        
        private DialogFlow HideoutBossDialog()
        {
            DialogFlow dialogFlow = DialogFlow.CreateDialogFlow("start", 125).NpcLine(GameTexts.FindText("rf_third_quest_boss_dialog_1")).Condition(() => CharacterObject.OneToOneConversationCharacter?.StringId == hideoutBossCharacterId && Settlement.CurrentSettlement == questHideout && goToHideoutLog?.CurrentProgress == 2)
