...
public class FloatingTextMissionView : MissionView
{
    ...
    public override void OnBehaviorInitialize()
    {
        ...
        foreach (var entry in FloatingTextManager.Instance.Entries)
        {
            ...
            _vm.AddTextItem(new FloatingTextItemVM(entry.Key, pos, text, entry.Value.Color, false));
        }
    }
    public override void OnMissionTick(float dt)
    {
        ...
        foreach (var entry in FloatingTextManager.Instance.Entries)
        {
            if (!_vm.ContainsKey(entry.Key))
            {
                ...
                _vm.AddTextItem(new FloatingTextItemVM(entry.Key, pos, text, entry.Value.Color, isVisible && canSeeTexts));
            }
        }
