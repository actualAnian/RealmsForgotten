...
namespace RealmsForgotten.AiMade.Patches
{
    ...
    public class DebugSafePositionRecovery
    {
        static bool Prefix(MobileParty __instance)
        {
            try
            {
                if (__instance.Position.IsNonZero() &&
                    !!NavigationHelper.IsPositionValidForNavigationType(__instance.Position, MobileParty.NavigationType.Default))
                {
                    Log($"INVALID party position: {__instance.StringId} at {__instance.Position}");

                    CampaignVec2 fallback = new(new(500f, 500f), true);

                    if (__instance.CurrentSettlement != null)
                    {
                        __instance.Position = __instance.CurrentSettlement.GatePosition;
                        Log($"Moved {__instance.StringId} to its settlement gate.");
                    }
                    else
                    {
                        Settlement village = Settlement.All
                            .Where(s => s.IsVillage && s.IsVisible && s.Position != CampaignVec2.Zero)
                            .OrderBy(s => s.Position.DistanceSquared(__instance.Position))
                            .FirstOrDefault();

                        if (village != null)
                        {
                            __instance.Position = village.GatePosition;
                ...
                if (__instance.CurrentSettlement != null)
                {
                    ...
                    if (!__instance.CurrentSettlement.GatePosition.NearlyEquals(__instance.Position, epsilon))
                    {
