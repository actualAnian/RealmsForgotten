...
namespace RealmsForgotten.Behaviors
{
    ...
    public class PotionsMissionBehavior : MissionBehavior
    {
        ...
        public override void OnMissionTick(float dt)
        {
            ...
            if (Agent.Main == null)
                return;
            if (elixir.Amount > 0 && Input.IsKeyReleased(SubModule.Instance.KeysConfig[nameof(RFSettings.Instance.UseHealKey)]) && Agent.Main.Health < 100)
            {
                DrinkElixir();
            }

            if (berserker.Amount > 0 && Input.IsKeyReleased(SubModule.Instance.KeysConfig[nameof(RFSettings.Instance.UseBerserkerKey)]))
            {
