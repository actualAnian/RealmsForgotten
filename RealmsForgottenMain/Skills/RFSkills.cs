...
namespace RealmsForgotten.CustomSkills
{
    public class RFSkills
    {
        ...
        public void Initialize()
        {
            ...
            _faith.Initialize(new TextObject("{=faith}Faith", null), new TextObject("{=faith_desc}Faith is your deeply held belief on  your chosen religion or a deep trust on your spiritual convictions."), new CharacterAttribute[] { RFAttributes.Discipline });

            _arcane = Game.Current.ObjectManager.RegisterPresumedObject(new SkillObject("arcane"));
            _arcane.Initialize(new TextObject("{=arcane}Arcane", null), new TextObject("{=arcane_desc}Represents your knowledge in the ancient rites and supernatural phenomena, including the use of organic and inorganic materials in incantations. It defines your capacity to to access magic."), new CharacterAttribute[] { RFAttributes.Discipline });

            _alchemy = Game.Current.ObjectManager.RegisterPresumedObject(new SkillObject("alchemy"));
            _alchemy.Initialize(new TextObject("{=alchemy}Alchemy", null), new TextObject("{=alchemy_desc}Alchemy represents your  understanding in manipulating matter and mixing base substances into higher or more purified forms."), new CharacterAttribute[] { RFAttributes.Discipline });
        }
        public RFSkills()
        {
            Instance = this;
    ...
    public class RFSkillEffects
    {
        ...
        public void InitializeAll()
        {
            ...
            _wandReloadSpeed.Initialize(new TextObject("{=arcane_skilleff_1}Wand reload speed: +{a0} %", null), RFSkills.Arcane, PartyRole.Personal, 0.02f, EffectIncrementType.AddFactor);
            _wandAccuracy.Initialize(new TextObject("{=arcane_skilleff_2}Wand accuracy: +{a0} %", null), RFSkills.Arcane, PartyRole.Personal, 0.02f, EffectIncrementType.AddFactor);
            _magicStaffPower.Initialize(new TextObject("{=arcane_skilleff_3}Magic staff power: +{a0} %", null), RFSkills.Arcane, PartyRole.Personal, 0.04f, EffectIncrementType.AddFactor);
            _faithPerkMultiplier.Initialize(new TextObject("{=faith_skilleff_1}Perk effect multiplier: +{a0} %", null), RFSkills.Faith, PartyRole.Personal, 0.04f, EffectIncrementType.AddFactor);
            _bombStackMultiplier.Initialize(new TextObject("{=alchemy_skilleff_1}Bomb stack multiplier: +{a0} %", null), RFSkills.Alchemy, PartyRole.Personal, 0.04f, EffectIncrementType.AddFactor);
        }
