...
using RealmsForgotten.Smithing.Patches;
...
public class CraftingMixin : BaseViewModelMixin<CraftingVM>
{
    ...
    public static void ApplyPatches(Harmony harmony)
    {
        ...
        harmony.Patch(
            ...
            postfix: new HarmonyMethod(AccessTools.Method(typeof(CraftingMixin), nameof(ModePropertiesPostfix))));
        harmony.Patch(
            typeof(Crafting).Assembly.GetTypes().First(t => t.Name.Contains("CraftingStats")).GetRuntimeMethods().First(m => m.Name == "CalculateStats"),
            postfix: new HarmonyMethod(AccessTools.Method(typeof(CalculateStatsPatch), "Postfix")));
    }
    ...
    public static CraftingMixin? Instance { get; private set; }
    ...
    public CraftingMixin(CraftingVM vm) : base(vm)
    {
        ...
        _armorText = GameTexts.FindText("str_rfsmithing_crafting_category_armor").ToString();
    ...
    public static bool IsMainActionExecuting = false;
    [DataSourceMethod]
    ...
    public void ExecuteSwitchToArmor()
    {
        ...
        string t = GameTexts.FindText("str_rfsmithing_crafting_category_armor").ToString();
    ...
    public void OnCraftingLogicRefreshed(Crafting newCraftingLogic)
    {
        ...
        AccessTools.Method(typeof(WeaponDesignVM), "OnCraftingLogicRefreshed").Invoke(CraftingVm.WeaponDesign, new object[] { newCraftingLogic });
    }
    ...
    private void CraftItem(RFSmithingModel smithingModel, Hero hero, ItemObject item)
    {
    ...
    private void UpdateCurrentMaterialCosts()
    {
        ...
        if (!IsInArmorMode)
        {
            ...
            {
                RFSmithingModel tempSmithingModel = (RFSmithingModel)Campaign.Current.Models.SmithingModel;
    ...
    private void RefreshEnableMainAction()
    {
        ...
        if (!HaveEnergy(hero) && !noStaminaRequired)
        {
            ...
            ViewModel.MainActionHint = new BasicTooltipViewModel(() =>
                GameTexts.FindText("str_rfsmithing_crafting_stamina_display")
    ...
    private void SpendMaterials(WeaponDesign weaponDesign)
    {
        ...
        bool kardrathiumEnabled = WeaponDesignMixin.Instance?.KardrathiumButtonToggle?.UseKardrathium ?? false;
        for (int i = 0; i < smithingCostsForWeaponDesign.Length; i++)
        {
            if (smithingCostsForWeaponDesign[i] != 0)
            {
                if (kardrathiumEnabled && KardrathiumButtonToggleVM.Irons.Contains((CraftingMaterials)i))
                {
                    smithingCostsForWeaponDesign[i] = 0;
                }
                ItemObject itemObject = smithingModel.GetCraftingMaterialItem((CraftingMaterials)i);
    ...
    private void SpendMaterials()
    {
        ...
        if (baseSmithingModel is RFSmithingModel smithingModel)
        {
            ...
            for (int i = 0; i < (int)CraftingMaterials.NumCraftingMats; i++)
            {
                if (kardrathiumEnabled && KardrathiumButtonToggleVM.Irons.Contains((CraftingMaterials)i))
                {
                    smithingCostsForArmorCrafting[i] = 0;
                }
