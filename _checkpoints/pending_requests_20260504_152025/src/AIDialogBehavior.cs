        ...
        private void ConsequenceClearResponse()
        {
            ...
            if (_currentContext != null && _parsed != null)
            {
                    ...
                    !string.IsNullOrWhiteSpace(_parsed.PersonalitySummary))
                {
                    ...
                    // Named ambition — lords only, generated once and kept forever
                    if (!string.IsNullOrWhiteSpace(_parsed.Ambition)
                        && _currentNpc?.IsLord == true)
                    {
                ...
                // Store long-term memory note if LLM flagged this exchange as significant
                if (!string.IsNullOrWhiteSpace(_parsed.MemoryNote))
                {
                    ...
                    _currentContext.AddMemory(_parsed.MemoryNote, day);
                }

                // ── Pending request lifecycle ─────────────────────────────
                // New request made this turn → save it
                if (!string.IsNullOrWhiteSpace(_parsed.Request))
                {
                    ...
                    {
                        Description = _parsed.Request!.Trim(),
                        DayIssued   = day
                    };
                }

                // Request fulfilled this turn → clear it
                ...
                // Clear initiative — it was addressed in this conversation
