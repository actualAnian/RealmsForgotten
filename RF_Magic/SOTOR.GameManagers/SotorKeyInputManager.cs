...
public static class SotorKeyInputManager
{
	...
	private static void RegisterKeybindStrings()
	{
		try
		{
			...
			{
				gameTextManager.GetGameText("str_key_category_name").AddVariationWithId("SotorGameKeyContext", new TextObject("{=sotor_key_category_name}Spells of the Old Realms"), new List<GameTextManager.ChoiceTag>());
