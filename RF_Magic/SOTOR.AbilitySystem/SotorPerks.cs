...
public class SotorPerks
{
	...
	private void InitializeAll()
	{
		...
		_entrySpells.Initialize("Thyrn-Kael", spellcraft, 25, null, "Gain access to entry level spells.", PartyRole.Personal, 0f, EffectIncrementType.Invalid);
		_adeptSpells.Initialize("Kaeth-Vorn", spellcraft, 125, null, "Gain access to adept level spells. Unlocks the ability to learn the Lore of Necromancy.", PartyRole.Personal, 0f, EffectIncrementType.Invalid);
		_masterSpells.Initialize("Vorn-Aer", spellcraft, 200, null, "Gain access to master level spells.", PartyRole.Personal, 0f, EffectIncrementType.Invalid);
		_selfish.Initialize("Sael-Vorn", spellcraft, 75, _wellControlled, "Your damaging spells do 90% reduced damage to yourself.", PartyRole.Personal, -0.9f, EffectIncrementType.AddFactor, "Your self targeted buff spells have 50% more duration.", PartyRole.Personal, 0.15f, EffectIncrementType.AddFactor, TroopUsageFlags.None, TroopUsageFlags.None);
		_wellControlled.Initialize("Dôr-Kael", spellcraft, 75, _selfish, "Your damaging spells do 30% less damage to troops in your party.", PartyRole.Personal, -0.3f, EffectIncrementType.AddFactor, "Gain 5% advantage in simulation battles.", PartyRole.PartyLeader, 0.05f, EffectIncrementType.AddFactor, TroopUsageFlags.None, TroopUsageFlags.None);
		_librarian.Initialize("Lôre-Maer", spellcraft, 275, _storyTeller, "You gain 25% more Spellcraft experience from casting spells.", PartyRole.Personal, 0.25f, EffectIncrementType.AddFactor, "Learning new spells and lores costs 50% less gold.", PartyRole.Personal, -0.5f, EffectIncrementType.AddFactor, TroopUsageFlags.None, TroopUsageFlags.None);
		_storyTeller.Initialize("Sâga-Hâld", spellcraft, 275, _librarian, "Every companion in your party gains 1000 experience in a random skill per day.", PartyRole.PartyLeader, 1000f, EffectIncrementType.Add, "Your party gains a permanent +5 increase to party morale.", PartyRole.PartyLeader, 5f, EffectIncrementType.Add, TroopUsageFlags.None, TroopUsageFlags.None);
		_overCaster.Initialize("Bael-Sûl", spellcraft, 175, _efficientSpellCaster, "Your instant damaging and healing spells are 20% more effective but cost 30% more mana.", PartyRole.Personal, 0.2f, EffectIncrementType.AddFactor, "", PartyRole.None, 0.15f, EffectIncrementType.AddFactor, TroopUsageFlags.None, TroopUsageFlags.None);
		_efficientSpellCaster.Initialize("Thael-Vorn", spellcraft, 175, _overCaster, "Your instant damaging and healing spells are 20% less effective, but cost 30% less mana.", PartyRole.Personal, -0.2f, EffectIncrementType.AddFactor, "", PartyRole.None, -0.15f, EffectIncrementType.AddFactor, TroopUsageFlags.None, TroopUsageFlags.None);
		_improvision.Initialize("Vel-Gaer", spellcraft, 225, _catalyst, "Your Mana is set to 25 if you have less than that at the beginning of the battle.", PartyRole.Personal, 25f, EffectIncrementType.Add, "+10% Persuasion chance during speech checks.", PartyRole.Personal, 0.1f, EffectIncrementType.AddFactor, TroopUsageFlags.None, TroopUsageFlags.None);
		_catalyst.Initialize("Ûr-Thyrn", spellcraft, 225, _improvision, "For every legendary item in your equipment slots you gain +5 extra mana at the start of battle.", PartyRole.Personal, 5f, EffectIncrementType.Add, "You gain +20% Mana regeneration while waiting in a town.", PartyRole.Personal, 0.2f, EffectIncrementType.AddFactor, TroopUsageFlags.None, TroopUsageFlags.None);
		_dampener.Initialize("Morg-Hael", spellcraft, 250, _arcaneLink, "Damage dealt by your damaging spells is reduced by 15%, but troops in your formation take 30% less damage from spells.", PartyRole.Personal, -0.15f, EffectIncrementType.AddFactor, "You gain 5% ward save.", PartyRole.Personal, -0.05f, EffectIncrementType.AddFactor, TroopUsageFlags.None, TroopUsageFlags.None);
		_arcaneLink.Initialize("Elda-Thael", spellcraft, 250, _dampener, "Any buffs you cast on a friendly unit will now also apply to you even if you are not in range.", PartyRole.Personal, 1f, EffectIncrementType.Add, "As formation Captain, all troops in your formation deal additional 10% magic damage.", PartyRole.Captain, 0.1f, EffectIncrementType.AddFactor, TroopUsageFlags.None, TroopUsageFlags.None);
